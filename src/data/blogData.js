...
배포: 워크플로우 정의 push → CI 자동 배포
    ...
    {
        id: 'asset-identity-split',
        domains: ['pipeline', 'backend'],
        tag: 'Pipeline',
        title: '한글 표시명 ↔ 영문 폴더 코드 — 사람과 시스템이 각자의 이름을 쓰는 에셋 아이덴티티',
        problem: '아티스트는 에셋을 한글 이름으로 부르는데, NAS 폴더·엔진 경로·SOP는 영문 코드만 허용한다. 하나의 이름 필드로 둘을 겸하게 두자 한글이 폴더 경로에 섞여 깨진 디렉토리(garbage 경로)가 생기고, 요청 폼에서 상위작업(부모)을 고를 때 그 항목이 무엇을 가리키는지 분간이 안 됐다.',
        solution: '표시명(한글)과 폴더 코드(영문 SOP)를 별도 필드로 분리해 카탈로그가 둘을 모두 보유하게 했다. 사람이 보는 모든 UI(요청 폼·배정 알림·리포트)는 한글 표시명을, 시스템이 만드는 모든 경로(NAS 폴더·작업목록 키·엔진 반영 키)는 영문 코드를 쓴다. 요청 폼의 부모 드롭다운은 작업목록이 아니라 카탈로그를 소스로 삼아 "한글명 + 코드"를 함께 보여주고, 폴더 코드는 사람이 입력하지 않고 코드 기반으로 자동 생성해 오타·한글 혼입을 원천 차단한다. 입력단에서 한글·공백·특수문자는 타이핑 즉시 제거하고 백엔드가 동일 규칙으로 재검증한다.',
        insight: '이름은 한 개가 아니라 "청중별로 다른 여러 개"다. 사람의 이름과 시스템의 키를 한 필드에 겹치면 반드시 한쪽이 깨진다. 표시명과 코드를 분리하고 코드를 자동 파생시키면, 사람은 읽기 좋은 이름을 쓰면서도 경로·키는 항상 안전한 불변식을 유지한다.',
        arch: `Asset Identity (표시명 ↔ 코드)
카탈로그 = { 한글 표시명, 영문 폴더 코드 } 동시 보유
├── 사람 UI (요청 폼·배정 알림·리포트) → 한글 표시명
├── 시스템 경로 (NAS 폴더·작업목록 키·엔진 반영 키) → 영문 코드
├── 부모 드롭다운 소스 = 카탈로그 ("한글명 + 코드" 동시 표기)
└── 코드 = 자동 생성 (수동 입력 X) → 한글·오타 혼입 원천 차단
입력 검증: 프론트 즉시 제거 + 백엔드 재검증 (2중)`
    },
    {
        id: 'tasks-board-sync',
        domains: ['pipeline', 'backend'],
        tag: 'DevOps',
        title: '내부 작업목록 ↔ 팀 보드 양방향 동기화 — 한 번 등록, 어디서 봐도 같은 상태',
        problem: '파이프라인 내부 작업목록과 팀이 실제로 들여다보는 협업 보드가 따로 놀았다. 새 작업을 양쪽에 수동 등록하면 하나가 빠지거나 상태가 어긋나고, 담당자가 보드에서 조정한 작업일자는 파이프라인에 반영되지 않았다.',
        solution: '내부 작업목록을 정본으로 두고 팀 보드로 자동 등록·상태를 push 하는 동시에, 팀원이 보드에서 바꾼 작업일자를 pull 로 되가져오는 양방향 동기화를 무인 사이클에 편입했다. 동기화 스코프는 수동 등재를 폐지하고 요청 DB에서 대상 보드를 자동 판정하도록 바꿨고, 신규 소품·배경 보드가 누락되던 문제는 라우팅을 설정 파일로 이관해 해소했다. 모든 연산은 멱등 — 주기가 겹쳐 재실행·중복 등록이 일어나도 같은 키로 수렴한다.',
        insight: '두 시스템을 잇는 동기화의 성패는 "방향"과 "멱등성"이다. 어느 쪽이 정본이고 어떤 필드가 어느 방향으로 흐르는지를 필드 단위로 못 박고, 모든 연산을 키 기반 멱등으로 설계하면, 크론 주기가 겹쳐 두 번 돌아도 상태가 진동하지 않는다.',
        arch: `작업목록 ↔ 팀 보드 양방향 Sync (무인 사이클)
내부 작업목록 (정본 SSOT)
├── push → 팀 보드 자동 등록 + 상태 sync
├── pull ← 팀 보드에서 조정한 작업일자 회수
├── 스코프 = 요청 DB 자동 판정 (수동 등재 폐지)
├── 라우팅 = 설정 파일 (신규 소품·배경 보드 누락 해소)
└── 전 연산 멱등 (키 기반, 재실행·중복 안전)`
    },
    // ══════════════════════════════════════
    // XROOM — 960+ commits
    // ══════════════════════════════════════
...
└── Unreal (Python)
    ...
    {
        id: 'preview-bus-switching',
        domains: ['frontend', 'engine'],
        tag: 'Broadcast',
        title: '"누르면 곧 송출"을 없앤 원웨이 스위칭 — PVW 경유 CUT 버스',
        problem: '물리 컨트롤러의 카메라 키가 채널 소스를 직접 바꾸고 있었다. 즉 키 하나를 잘못 누르면 그 순간 방송 화면이 바뀐다. 게다가 탈리(적/녹) 표시를 소스별 플래그로 관리해 메인/백업 두 대가 서로의 잔상을 지우지 못하고 두 곳이 동시에 켜지는 일이 있었다.',
        solution: '카메라 키를 PVW(프리뷰) arm 전용으로 바꾸고, 송출은 CUT 키 하나만 담당하게 했다. 빈 PVW 상태에서는 CUT을 거부한다. FTB가 없는 구성이라 화면을 끊는 BLACK도 예외로 두지 않고 똑같이 PVW에 올린 뒤 CUT으로 내보낸다 — 무엇이 나갈지 멀티뷰에서 확인하고 누르게 된다. 탈리는 플래그가 아니라 정체(identity) 피드백으로 재설계했다: 버스가 현재 프로그램/프리뷰 소스 ID를 방송하면 각 키가 자기 ID와 비교해 색을 정한다. 키별 구독을 없애고 전역 1회 구독 + 전체 리페인트로 잔상을 원천 차단.',
        insight: '오조작을 줄이는 방법은 경고를 붙이는 게 아니라 "누름 = 송출"이라는 경로 자체를 제거하는 것이다. 그리고 상태 표시는 각 클라이언트가 켜고 끄는 플래그가 아니라 "지금 나가는 건 누구인가" 하나를 받아 각자 자기와 비교하게 해야, PC가 몇 대든 표시가 갈라지지 않는다.',
        arch: `One-Way Switching Bus
Camera Key ─┐
BLACK Key  ─┼─→ /bus/pvw/source  (arm only)
Preset Key ─┘         │
                      ▼
                 [ PVW armed? ] ──no──→ CUT 거부
                      │yes
                      ▼
                 CUT Key ──→ /bus/program/take
                      │
                      ▼
        bus/{program|preview}/source  (identity broadcast)
                      │
        ┌─────────────┴─────────────┐
        ▼                           ▼
   Key.id == program ? RED     Key.id == preview ? GREEN
   (전역 1회 구독 → 전체 리페인트)`
    },
    {
        id: 'watchdog-crash-loop',
        domains: ['frontend', 'engine'],
        tag: 'Broadcast',
        title: '40초마다 죽는 플러그인 — 워치독이 죽인 부팅 워밍업',
        problem: '방송 컨트롤러 플러그인이 약 40초마다 강제 종료 → 재시작을 무한 반복했다. 현장에서 관측되는 증상은 "모든 버튼이 계속 깜박인다"뿐이었다.',
        solution: '부팅 시 실행하던 스위처 클라이언트 워밍업이 원인이었다. 스위처 라이브러리를 단일 스레드 모드로 돌리는 상태에서 4K 스위처가 초기 미디어풀·오디오·매크로 상태를 한꺼번에 덤프하면, 그 파싱이 메인 스레드에서 이벤트 루프를 40초가량 막는다. 컨트롤러 런타임의 30초 워치독은 이를 "응답 없음"으로 보고 프로세스를 죽이고, 재시작하면 같은 워밍업이 다시 발화해 사이클이 닫힌다. 스위처 클라이언트는 원래 lazy 싱글턴이라 워밍업 호출만 걷어내면 첫 캡처 시점에 1초 지연 한 번으로 알아서 연결된다. 카메라·OSC·피드백 액션은 스위처와 무관해 영향이 없다.',
        insight: '워치독이 있는 런타임에서 "미리 데워두기"는 최적화가 아니라 자살 버튼이 될 수 있다. 블로킹 파싱을 감춘 초기화를 부팅 경로에 올리면, 느려지는 게 아니라 아예 살아남지 못한다. 근본 해법(별도 프로세스 분리)을 넣기 전까지는 지연 1초를 받아들이는 lazy 초기화가 옳은 선택이었다.',
        arch: `Crash Cycle
boot → warmSwitcherClient()
        └→ switcher state dump (mediapool/audio/macro)
             └→ single-thread parse ≈ 40s block
                  └→ runtime watchdog (30s) → SIGKILL
                       └→ restart → (same boot path) ↺

Fix
boot → (no warm-up)
first capture press → lazy singleton connect (~1s, once)
camera / OSC / feedback → switcher-independent`
    },
    {
        id: 'button-action-library',
        domains: ['frontend', 'engine'],
        tag: 'Broadcast',
        title: '물리 버튼 액션 라이브러리 — 이름으로 부르는 포즈, 자동 미러 fan-out',
        problem: '방송 중 필요한 조작이 카메라 전환을 넘어 계속 늘어났다(포즈 회전·액세서리·라디오·프리셋·시퀀스·슬라이드·자료 화면). 액션을 늘릴수록 두 가지가 걸렸다 — ① 엔진 쪽 배열 인덱스로 버튼을 지정하면 배열 순서가 바뀔 때마다 버튼이 통째로 어긋나고, ② 메인/백업 인스턴스를 함께 제어하려면 외부 브리지 소프트웨어가 필요했다.',
        solution: '액션 계층을 정리했다. 회전 액션은 인덱스가 아니라 포즈 **이름**으로 지정해 엔진의 포즈 배열 순서가 바뀌어도 버튼이 틀어지지 않게 했다. 액세서리처럼 월드 액터로 주소지정할 수 없는 대상은 캐릭터+슬롯 주소 체계를 따로 뒀다. 상태가 없는 액션(지정 포즈/다음 포즈/상대 회전/라디오 튜닝)은 stateless로 유지해 멀티액션에 안전하게 섞이고, 연속 회전처럼 상태가 있는 것만 피드백을 구독한다. 전송단은 sendOSC 하나로 모으고 primary 전송 뒤 세컨더리 대상들에게 같은 포트로 미러 UDP를 보내는 fan-out을 넣어, 외부 브리지 없이 플러그인만으로 멀티 서버를 제어한다. 세컨더리 실패는 로그만 남기고 primary를 막지 않는다. 카메라 소스 이름 목록도 별도 파일을 없애고 프리셋 JSON 하나(schemaVersion 3)에서 읽어 버튼 라벨과 설정 패널이 같은 진실을 본다.',
        insight: '버튼은 "몇 번째"가 아니라 "무엇"을 눌러야 오래 간다. 인덱스 주소지정은 만드는 순간이 가장 편하고 그 뒤로는 계속 비용이 된다. 그리고 fan-out은 전송 함수 한 곳에만 넣으면 기존 액션 전부가 공짜로 미러링된다 — 액션마다 미러 로직을 복사했으면 액션이 늘 때마다 누락이 생겼을 것이다.',
        arch: `Action Layer
├── stateless (multi-action safe)
│   ├── pose by NAME  ← 배열 순서 변경에 불변
│   ├── next pose / relative turn
│   ├── radio tune (index | name | next | random)
│   └── accessory toggle (character + slot 주소)
└── stateful (feedback 구독)
    └── continuous spin, preview/program tally

Transport
sendOSC(addr, args)
 ├── primary host:port  (action settings)
 └── secondary hosts[]  ← 미러 UDP (실패는 로그만)
      ⇒ 기존 컨비니언스 함수 전부 자동 미러

Source of truth
preset.json (schemaVersion 3)
 ├── cameras[]  → 프리셋 저장/불러오기
 └── sources[]  → 버튼 라벨 · 설정 패널 목록 (FreeCam 포함)`
    },
    {
        id: 'notify-fatigue-redesign',
        domains: ['pipeline', 'backend'],
        tag: 'DevOps',
        title: '알림 피로도 재설계 — 제외목록 대신 상태를 축으로',
        problem: '무인 검증 알림이 매일 같은 목록을 재탕했다. 한쪽은 아직 시작조차 안 한 공정을 "디렉토리 없음" 위반으로 매일 울렸고(16건 + 리마인더 15행), 다른 한쪽은 이미 폐기·보류된 작업의 위반이 매일 미해결 목록에 다시 올라왔다. 알림이 많아지면 사람들은 전부를 무시하기 시작한다 — 진짜 위반까지 같이 묻힌다.',
        solution: '알림 쪽에 제외목록을 두는 대신 두 축을 바로잡았다. ① 특정 공정은 애초에 제작 단계의 산출물이 아니라 후속 작업이 시작될 때 비로소 생기는 일이므로, 비어 있는 것이 정상 상태다 — 필수에서 선택으로 내리고 작업 자동 생성에서 제외해 "위반"이 아니라 "아직 배정되지 않은 일"로 다뤘다. 알림원이 스캐너와 리마인더 둘이라 양쪽 미러를 함께 껐고, 세 미러(파이썬·PHP·워크플로 JSON)를 한 테스트가 같이 검증해 한쪽만 바뀌면 실패하게 했다. ② 보류·폐기 작업의 위반은 작업 상태를 그대로 축으로 삼아 일일 알림·에스컬레이션에서 빼고 주 1회 결산에만 상세를 싣는다. 판정 단위가 에셋×변형이라 위반 메시지 머리의 변형 경로를 떼어 작업목록과 대조하고, 실재하는 변형일 때만 강등한다(오탐으로 알림이 꺼지지 않게). 조회가 실패하면 전건 활성으로 폴백해 알림이 조용히 죽지 않는다.',
        insight: '알림을 줄이려고 알림 쪽에 예외 목록을 만들면 진실이 두 개가 된다. 줄여야 할 것은 알림이 아니라 "위반의 정의"다 — 공정이 언제 생기는 일인지, 그 작업이 지금 살아 있는지를 원래 상태값으로 판정하면 예외 목록 없이도 조용해지고, 상태를 되돌리면 경과일이 끊기지 않고 그대로 이어진다.',
        arch: `Before — 알림 축이 없음
NAS scan ─┐
reminder ─┴→ 매일 같은 미해결 목록 (보류·폐기·미착수 전부 포함)

After — 상태가 곧 축
공정 정의: required → optional (on-demand)
   └ 폴더는 계속 생성, 내용이 생기면 포맷 검증 그대로
작업 상태: 보류 / 폐기 추가
   ├─ 일일 알림 · 에스컬레이션 · 주간 통계 → 제외 (요약줄 카운트만)
   └─ 주 1회 결산 → 상세 (보류만 남은 방도 주간엔 깨움)
판정 단위: 에셋 × 변형
   └ 위반 메시지 → 변형 경로 분리 → 작업목록 대조 → 실재할 때만 강등
조회 실패 → 전건 활성 폴백 (알림이 조용히 죽지 않게)`
    },
    {
        id: 'release-artifact-parity',
        domains: ['pipeline', 'backend'],
        tag: 'Delivery',
        title: '"내 PC에서만 안 돼요"를 구조로 없앤 릴리즈 패리티',
        problem: '신규 팀원이 배포 ZIP을 받아 처음 실행하면 항상 실패했다. 설치는 성공했다고 찍히는데 바로 다음 줄에서 모듈을 못 찾는다. 게다가 같은 버전인데 배포 채널에 따라 ZIP 내용물이 달랐고, 의존성은 전부 하한만 걸려 있어 설치 시점마다 다른 버전이 잡혔다. 셋 다 이미 설치가 끝난 개발 머신에서는 재현되지 않는 종류의 결함이었다.',
        solution: '세 층을 각각 막았다. ① 첫 실행 실패: editable 설치가 심는 경로 파일은 인터프리터 기동 시에만 처리되므로, 설치를 끝낸 그 프로세스에서 곧바로 import하면 못 찾는다 — 설치 후 사이트 경로를 현재 프로세스에 재적용하고 import 캐시를 무효화했다. 설치 판정도 버전 문자열 비교에서 "설치된 패키지의 실제 소스 경로가 지금 이 폴더인가"까지 확인하도록 바꿨다(같은 버전을 다른 폴더에 풀면 옛 폴더를 계속 바라보다 그 폴더를 지우는 순간 영구 고장이었다). 최후 방어선으로 그래도 안 되면 소스 경로를 직접 주입해 대시보드는 뜨게 하고 진단 명령을 함께 출력한다. ② 채널 불일치: 화이트리스트로 복사하던 릴리즈 패키징을 아카이브 산출 방식으로 통일해 두 채널이 같은 산출물을 내게 하고, 필수 파일 7종과 버전·태그 일치를 패키징 직후 검증해 어긋나면 릴리즈를 실패시켰다. ③ 의존성: 다음 메이저 전까지 상한을 고정하고, deprecated 이벤트 훅을 lifespan으로 옮겼다.',
        insight: '"내 PC에서는 되는데"의 대부분은 운이 아니라 구조다 — 기존 설치가 결함을 가려주기 때문에 개발 머신에서는 영원히 재현되지 않는다. 그래서 검증도 "돌려보기"가 아니라 빈 환경 + 갓 푼 ZIP이라는 최초 상태를 재현하는 쪽이어야 하고, 화이트리스트처럼 사람이 계속 관리해야 하는 목록은 언젠가 반드시 한 줄 빠진다.',
        arch: `Release Parity
[packaging]
  channel A (CI)   ─┐
  channel B (NAS)  ─┴→ 동일 archive 산출 (whitelist 폐기)
        └→ postcheck: 필수 파일 7종 + VERSION↔tag 일치
              └ 불일치 → 릴리즈 실패 (깨진 ZIP 배포 차단)

[first run]
  install editable → .pth (다음 기동에만 반영)
        └→ site 재적용 + import 캐시 무효화  ← 같은 프로세스에서 즉시 import 가능
  install 판정 = version AND 설치본 소스 경로 == 이 ROOT
        └ 최후 폴백: src 직접 주입 + 진단 출력

[dependencies]
  >=only  →  >=x,<next-major  (설치 시점 최신본 표류 차단)
  on_event("startup") → lifespan`
    },
    {
        id: 'conf-safe-write',
        domains: ['backend', 'pipeline'],
        tag: 'DevOps',
        title: '회원가입 한 번이 팀 전원의 접근을 끊을 수 있었다 — 설정 파일 안전 쓰기',
        problem: '팀 서버의 계정 등록이 버전관리 서버의 권한·계정 설정 파일을 텍스트로 편집하는 구조였는데, 여기에 팀 전원을 잠글 수 있는 경로가 네 군데 있었다. ① 파일 읽기 실패를 검사하지 않아, 실패값이 문자열 연결에서 빈 문자열로 취급되면 원본이 통째로 사라진 설정이 저장된다. ② 사용자 입력을 정규식 치환문에 그대로 넣어, $나 역슬래시가 든 비밀번호·이름이 역참조로 해석되면 계정이 조용히 다른 값으로 등록되거나 그룹 라인이 깨진다(= 그 그룹 전원 접근 불가). ③ 직접 덮어쓰기라 중간에 죽으면 반쯤 기록된 파일이 남는다. ④ 서버 기본 템플릿은 상단 주석에 섹션 이름 문자열을 그대로 포함하는데, 정규식이 그 주석에 먼저 매치돼 신규 계정이 어떤 섹션에도 속하지 않는 자리에 삽입됐다 — 파싱이 깨져 전원 인증 실패로 이어졌다.',
        solution: '읽기 실패와 빈 파일을 모두 "쓰기 중단"으로 처리하고, 모든 치환을 콜백 방식으로 바꿔 사용자 값이 치환문으로 해석될 여지를 없앴다. 쓰기는 백업 → 기존 섹션 소실 검사 → 임시 파일 → 원자적 이름 변경 순으로 재구성했고, 이름 변경이 디렉토리 권한 문제로 실패할 때만 직접 쓰기로 물러선다(그 시점엔 이미 검증과 백업을 마친 뒤다). 섹션 삽입은 정규식을 버리고 "줄 전체가 섹션 헤더인가"로만 판정해 주석 매치를 원천 차단하고, 줄 단위로 섹션 마지막 항목 뒤에 넣으며 개행 코드를 유지한다. 같은 지뢰가 아직 안 터졌을 뿐인 다른 함수도 함께 바꿨다.',
        insight: '설정 파일을 텍스트로 편집하는 코드의 핵심은 쓰기 로직이 아니라 "쓰지 않을 조건"이다. 읽기 실패·빈 결과·섹션 소실 중 하나라도 걸리면 멈춰야 하고, 저장은 항상 원자적 교체여야 한다. 그리고 정규식으로 구조를 찾는 순간 주석까지 구조로 보인다 — 형식이 정해진 파일은 정규식보다 줄 단위 파싱이 안전하다.',
        arch: `Safe Conf Write
read
 ├─ read failed?  → abort  (false → "" 연결 방지)
 └─ empty file?   → abort

mutate
 ├─ replace_callback()  ← 사용자 값이 역참조로 해석되지 않게
 └─ insert: 줄 전체가 "[section]" 인지로만 판정
      └ 섹션 마지막 항목 뒤 삽입 · CRLF/LF 보존

write
 ├─ .bak 백업
 ├─ 기존 섹션 소실 검사 → 하나라도 사라지면 abort
 ├─ tmp 파일 기록
 └─ rename (원자적 교체)
      └ rename 실패(디렉토리 권한) → 직접 쓰기 폴백 (검증·백업 후)`
    },
];
