...
7. 개발내용 최신화: 사용자가 "개발내용 최신화/소스 업데이트/하네스에서 업데이트" 라고 하면 /sync-dev 절차 실행 (회사 소스 프로젝트 → 익명화 → blogData/codeData, 초안 제안 후 승인). 실제 내부 명칭 절대 노출 금지.
8. 피드백 자동 축적 (컨텍스트 압축 후에도 유지):
